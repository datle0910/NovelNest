...
public class CrawlerController {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ApiResponse<com.example.novelnest.crawler.dto.CrawlProgress>> getLogProgress(@PathVariable Long id) {
        com.example.novelnest.crawler.dto.CrawlProgress progress = crawlerService.getProgress(id);
        if (progress == null) {
            // If not in map, might be finished or not started
            return importLogRepository.findById(id).map(log -> {
                com.example.novelnest.crawler.dto.CrawlProgress fallback = com.example.novelnest.crawler.dto.CrawlProgress.builder()
                        .logId(log.getId())
