...
import { Edit, Trash2, Check, X, Plus, Tags } from 'lucide-react';
...
const CategoryManagementPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Lỗi lưu thể loại');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 border-b border-white/5 pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-display font-extrabold text-white tracking-tight">Quản Lý Thể Loại</h1>
            <p className="text-xs text-text-muted font-semibold mt-1">Tạo mới, sửa thông tin và phân loại thể loại cho tác phẩm.</p>
          </div>
        </div>
        {!formVisible && (
          <button 
      ...
      {formVisible && (
        <div className="glass-card p-5 border-brand-500/20">
          <h3 className="text-sm font-bold text-white mb-3">
            {editingId ? 'Sửa thông tin thể loại' : 'Tạo thể loại mới'}
          </h3>
          <form onSubmit={handleSubmit} className="flex flex-col md:flex-row gap-4 items-start md:items-end">
            <div className="flex-1 w-full">
              <label className="block text-xs font-bold text-text-muted mb-1.5 uppercase tracking-wider">Tên thể loại *</label>
              <input
                type="text"
                required
            ...
            <div className="flex-[2] w-full">
              <label className="block text-xs font-bold text-text-muted mb-1.5 uppercase tracking-wider">Mô tả thể loại</label>
              <input
                type="text"
                placeholder="Nhập mô tả chi tiết của thể loại này..."
                value={description}
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Hiện tại hệ thống chưa có thể loại nào."
          icon={<Tags className="w-10 h-10 text-text-muted" />}
          actionLabel="Tạo Thể Loại Đầu Tiên"
              ...
              <tbody className="divide-y divide-white/5">
                {categories.map((cat) => (
                  <tr key={cat.id} className="hover:bg-white/[0.02] transition-colors">
                    <td className="px-6 py-4 font-bold text-white">{cat.id}</td>
                    <td className="px-6 py-4 font-semibold text-white">{cat.name}</td>
                    <td className="px-6 py-4 font-medium text-text-muted">{cat.slug}</td>
                    <td className="px-6 py-4 font-medium max-w-xs truncate">{cat.description || 'Chưa có mô tả'}</td>
      ...
      {deleteTarget && (
        <ConfirmModal
          isOpen={!!deleteTarget}
          title="Xóa Thể Loại"
          message={`Bạn có chắc chắn muốn xóa thể loại "${deleteTarget.name}" không? Hành động này không thể khôi phục.`}
